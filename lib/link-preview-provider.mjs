const METADATA_BASE_URL = 'https://api.microlink.io'
...
export function linkPreviewProviderUrl(endpoint, target) {
  const url = publicHttpUrl(target)
  if (!url) return null

  if (endpoint === 'favicon') {
    const provider = new URL(FAVICON_BASE_URL)
    provider.searchParams.set('domain_url', url.origin)
    provider.searchParams.set('sz', '64')
    return provider.href
  }

  const provider = new URL(METADATA_BASE_URL)
  provider.searchParams.set('url', url.href)
  if (endpoint === 'image') provider.searchParams.set('embed', 'image.url')
  return provider.href
}
...
export function normalizeOgMetadata(target, metadata, previous) {
  ...
  const payload =
    typeof metadata.data === 'object' && metadata.data !== null ? metadata.data : metadata

  const title = localizedField(
    payload.title ?? payload.ogTitle,
    previous?.title,
    previous?.titleEn,
  )
  const description = localizedField(
    payload.description ?? payload.ogDescription,
