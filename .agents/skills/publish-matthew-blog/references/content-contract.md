...
a `codex/preview/<slug>` branch. Push that branch and wait for the `Deploy Preview`
workflow. Other feature branches intentionally do not deploy the shared Preview.
...
Publishing keeps the registration, passes the scope-appropriate publication
validator and protected PR checks, merges to `main`, and waits for the Production
workflow. “Direct publish” may skip Matthew's Preview approval wait, but never
skips its focused or full validation gate, branch protection, Production
deployment, or hosted verification.
